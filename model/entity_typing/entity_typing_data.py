...
def tokenize(text, tokenizer, max_seq_length):
    textraw = text.split('\n')
    for delimiter in EntityMarkers:
        ntextraw = []
...
class ETDataset(IterableDataset):
    ...
    def __init__(self, src_file, save_file, max_seq_length, tokenizer):
        ...
        if os.path.exists(save_file):
            ...
            bertsProcessor_class = ETProcessor(src_file)
            ...
            # save entity dictionary of test set
            if "test" in save_file:
                self.entity_dict = []
                for f in features:
                ...
                with open(file=src_file + "/test_entity_dict.json", mode="w", encoding="utf8") as f:
                    json.dump(self.entity_dict, f)
                print('save entity dictionary to {}.'.format(src_file + "/test_entity_dict.json"))
            # save datasets
...
class ETDataLoader(DataLoader):
    def __init__(self, dataset, batch_size, shuffle=False, relation_num=5, max_length=512):
        super(ETDataLoader, self).__init__(dataset, batch_size=batch_size)
